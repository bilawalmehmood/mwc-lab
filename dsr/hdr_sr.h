...
#ifndef sr_hdr_h
#define sr_hdr_h
#include <iostream> // OCEAN - DEBUGGING
#include <assert.h>
#include <packet.h>
#include <list> // OCEAN
...
public:
	...
	// ************* OCEAN ************* //
	inline list<nsaddr_t>* getFaultyList() {
	  return faultyList;
	}

	void appendToFaultyList(list<nsaddr_t>* newFaultyList) {
	  list<nsaddr_t>::iterator iter = newFaultyList->begin();
	  while(iter != newFaultyList->end()) {
	    nsaddr_t temp_addr = *iter;
	    faultyList->push_back(temp_addr);
	    iter++;
	  }
	}
