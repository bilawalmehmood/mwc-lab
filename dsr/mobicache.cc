...
public:
  ...
  // it from the cache
  // ******************** OCEAN ********************* //
  void deleteRoute(nsaddr_t addr);
  // ****************** END OCEAN ******************* //
...
// ******************** OCEAN ********************* //
void MobiCache::deleteRoute(nsaddr_t addr) { 
  primary_cache->deleteRoute(addr);
  secondary_cache->deleteRoute(addr);
}
// ****************** END OCEAN ******************* //
...
void Cache::deleteRoute(nsaddr_t addr) {
  for(int p = 0; p < size; p++) {
    for(int n = 0; n <  (cache[p].length()-1); n++) {
      if(cache[p][n].getNSAddr_t() == addr) {
	cache[p].reset();
#ifdef ROUTE_RANKER_DEBUG
	cout << "  [" << Scheduler::instance().clock() << "] deleted route containing node #" << addr << " from my route cache" << endl << flush;
#endif
	break;
      }
