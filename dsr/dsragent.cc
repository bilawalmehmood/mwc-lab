...
bool dsragent_snoop_source_routes = false; // OCEAN
...
bool dsragent_send_grat_replies = false; // OCEAN
// should we send gratuitous replies to effect route shortening?
bool dsragent_salvage_with_cache = false; // OCEAN
...
bool dsragent_reply_from_cache_on_propagating = false; // OCEAN
// should we consult the route cache before propagating rt req's and
// answer if possible?
bool dsragent_ring_zero_search = false; // OCEAN
...
Our strategy is as follows:
 ...
 route request.  the code assumes that the only thing that remons2 how to set wireless radiusves
...
void BankTimer::expire(Event* e) {
  if(!agent->getFaultyStatus()) {
    agent->incrementAllBankEntries();
  }
  agent->resetBankTimer();
}

void FaultyTimer::expire(Event* e) {
  int current_time = (int)Scheduler::instance().clock();
  FaultyTimePeriod faultyPeriod = faultyTimeQueue.front();
  // if its faulty
  if(agent->getFaultyStatus()) {

    // check if the current time is the same as the end_time
    // of the faulty period
    if(current_time == faultyPeriod.getEndTime()) {
      // if it is, then set the node back to normal behaviour
      agent->setFaultyStatus(false);
      // and remove this faulty period from the queue
      faultyTimeQueue.pop();
    }
  }
  // if its not faulty
  else {
    // check if the current time is the same as the start_time
    // of the faulty period
    if(current_time == faultyPeriod.getStartTime()) {
      // if it is, then set the node to faulty behaviour
      agent->setFaultyStatus(true);
    }
  }
  
  // only reset the timer if its queue is not empty
  if(!faultyTimeQueue.empty()) {
    resched(1);
  }else {
    delete this;
  }
...
/*===========================================================================
  DSRAgent methods
---------------------------------------------------------------------------*/
DSRAgent::DSRAgent(): Agent(PT_DSR), myBank(), myMonitor(logtarget,&myBank,&route_cache), myBankTimer(this), request_table(128), route_cache(NULL), 
...
DSRAgent::command(int argc, const char*const* argv)
{
  ...
  trace("Sconfig %.5f $reply for props: %s ring 0 search: %s",
	  ...
	  // *************** OCEAN ***************** //
	  // start the bank timer
	  myBankTimer.resched(0.0);
...
#ifdef NODE_STATUS_DEBUG
		    cout << "Added faulty timer for node #" << net_id.dump() << " from start_time=" << startTime << "sec to end_time=" << endTime << "sec" << endl << flush;
#endif
		  }
		}
...
#ifdef NODE_STATUS_DEBUG
	  cout << "node #" << net_id.dump() << " isMalicious=" << isMalicious << " isSelfish=" << isSelfish << endl << flush;
#endif
   ...
   snooping as appropriate */
{
  ...
  if(isMalicious) {
    ...
    drop(p.pkt);
#ifdef NODE_STATUS_DEBUG
    cout << "[" << Scheduler::instance().clock() << "] node #" << net_id.dump() << " is faulty and so dropped packet received from node #" << srh->get_prev_addr() << endl << flush;
#endif
  }
  
  // if we are not malicious or selfish, use normal forwarding functionality
  else {
    if(myMonitor.faultyListContains(srh->get_prev_addr())) {
      // if the node requesting forwarding is in this node's list of faulty
      // nodes, then do not forward traffic.
      drop(p.pkt);
#ifdef MALICIOUS_TRAFFIC_REJECTION_DEBUG
      cout << "[" << Scheduler::instance().clock() << "] node #" << net_id.dump() << " dropped packet from node #" << srh->get_prev_addr() << " due to malicious traffic rejection" << endl << flush;
#endif
    }
    // if the node requesting forwarding is not in this node's list of faulty 
    // nodes, then continue with normal forwarding functionality
    else {
      if(srh->route_reply() || srh->route_request() || srh->get_prev_addr() != p.src.getNSAddr_t()) {
	// if its a route reply or a route request, or if the previous
	// node is forwarding for someone else (ie: is not the source), 
	// then dont charge credit, simply forward it onwards.
	sendOutPacketWithRoute(p, false);
      }else{
	// check if the node who sent us the packet is entered into our bank
	...
	// second: check if the node has enough credit to ask us to forward
	if(myBank.getChipCount(prev_hop) >= DECREMENT_AMOUNT) {
	  myBank.decChipCount(prev_hop);
#ifdef BANK_DEBUG
	  cout << "[" << Scheduler::instance().clock() << "] node #" << net_id.dump() << " forwarded packet for node #" << prev_hop << " $" << myBank.getChipCount(prev_hop) << " remaining." << endl << flush;
#endif
	  // third: increase the #packets we have forwarded for that node
	  ...
	  // if they dont have enough credit to forward, drop the packet
	  drop(p.pkt);
#ifdef BANK_DEBUG
	  cout << "[" << Scheduler::instance().clock() << "] node #" << net_id.dump() << " dropped packet since node #" << prev_hop << " has no $$" << endl << flush;
#endif
	}
      }
    }
  ...
  /* process a route request that isn't targeted at us */
{
  ...
  if(isSelfish) {
    ...
    drop(p.pkt);
#ifdef NODE_STATUS_DEBUG
    cout << "[" << Scheduler::instance().clock() << "] node #" << net_id.dump() << " selfishly dropped RREQ packet received from node #" << srh->get_prev_addr() << endl << flush;
#endif
    return;
  }
  else if(isFaulty) {
    drop(p.pkt);
#ifdef NODE_STATUS_DEBUG
    cout << "[" << Scheduler::instance().clock() << "] node #" << net_id.dump() << " is faulty and so dropped RREQ packet received from node #" << srh->get_prev_addr() << endl << flush;
#endif
    return;
  }
  else {
    // if we are not selfish or faulty, handle route-requests with normal behaviour:
    // get this node's faulty list
    list<nsaddr_t>* current_faulty_list = myMonitor.getFaultyList();
    list<nsaddr_t>::iterator iter = current_faulty_list->begin();
    
    while(iter != current_faulty_list->end() && iter != NULL) {
      // check if the current route contains nodes in this node's faulty list
      // if it does, then drop this route-request packet
      if(p.route.member(ID(*iter, ::IP))) {
#ifdef ROUTE_RANKER_DEBUG
	cout << "[" << Scheduler::instance().clock() << "] node #" << net_id.dump() << " dropped RREQ packet from node #" << p.src.getNSAddr_t() << " as route contained node in avoid list" << endl << flush;
#endif
	drop(p.pkt);
	return;
      }
     ...
     //  set as it currently is
{
  ...
  if (srh->route_request())
    { 
      // ******************** OCEAN ********************** //
      // we append this node's faulty list to the faulty list in the packet
      
      srh->appendToFaultyList(myMonitor.getFaultyList());

#ifdef ROUTE_RANKER_DEBUG
      cout << "[" << Scheduler::instance().clock() << "] node #" << net_id.dump() << " broadcasting RREQ originally from node #" << p.src.getNSAddr_t() << " after having added its own faulty list" << endl << flush;
#endif
    ...
    { // no jitter required 
      ...
      // we are sending a packet, so need to add it to this node's cache
      // only add it if we expect the next node to receive it to forward it,
      // ie, if the next node to receive is NOT the destination.
        
      // dont cache if the next hop is the destination (and hence the next guy wont forward it)
     ...
     - doesn't free the pkt */
{
  ...
  // **************** OCEAN **************** //
  // need to check if the route reply contains nodes in our
  // local faulty list.
  list<nsaddr_t>* faulty_list = myMonitor.getFaultyList();
  list<nsaddr_t>::iterator iter = faulty_list->begin();

  while(iter != faulty_list->end()) {
    // if it does contain a node in our avoid list, then we do not accept it
    if(reply_route.member(ID(*iter, ::IP))) {
#ifdef ROUTE_RANKER_DEBUG
      cout << "[" << Scheduler::instance().clock() << "] node #" << net_id.dump() << " did not accept route reply as it contained nodes in its faulty list" << endl << flush;
#endif
      return;
    }
    iter++;
  ...
  *** do not change or free packet *** */
{
  ...
  // OCEAN : i had to comment the previous line out to enable
  // the monitor to work correctly.

  // ****************** OCEAN ********************* //
  // dont check for route requests or route replies, since
  // we dont cache them anyway.
...
void DSRAgent::incrementAllBankEntries() {
#ifdef BANK_TIMER_DEBUG
    cout << "[" << Scheduler::instance().clock() << "] node #" << net_id.dump() << " timer will increment all bank entries with $" << BANK_TIMER_AMOUNT << endl << flush;
#endif
  myBank.incAllEntries(BANK_TIMER_AMOUNT);
}

void DSRAgent::resetBankTimer() {
  myBankTimer.resched(BANK_TIMER_FREQUENCY);
}

void DSRAgent::setFaultyStatus(bool val) {
  isFaulty = val;
#ifdef NODE_STATUS_DEBUG
  cout << "[" << Scheduler::instance().clock() << "] node #" << net_id.dump() << " faulty timer has changed its faulty status: isFaulty = " << isFaulty << endl << flush;
#endif
}

bool DSRAgent::getFaultyStatus() {
  return isFaulty;
}
