...
// **************** OCEAN ***************** //
// This class functions as a timer to periodically increment
// the chip count for all of a node's neighbours. This exists
// for nodes which may not have forwarding opportunities based
// on their location in the network, this timer will allow them
// to send traffic at a reduced rate.

class BankTimer : public TimerHandler {
 public: 
  BankTimer(DSRAgent* a) : TimerHandler() { agent = a; }
  void expire(Event* e);
...
class FaultyTimePeriod {
 public:
  FaultyTimePeriod(int s_t, int e_t) { start_time = s_t; end_time = e_t; }
...
class FaultyTimer : public TimerHandler {
 public:
  FaultyTimer(DSRAgent* a) : TimerHandler() { agent = a; }
  void addFaultyTimePeriod(FaultyTimePeriod p) { faultyTimeQueue.push(p); }
  void expire(Event* e);
  FaultyTimeQueue<FaultyTimePeriod> faultyTimeQueue;
...
public:
	...
	// ******************* OCEAN ********************* //
	// used by the BankTimer class
	void incrementAllBankEntries();
	void resetBankTimer();
	
	void setFaultyStatus(bool val);
	bool getFaultyStatus();
	void resetFaultyTimer();

	// used to read the nodes.info file containing info
	// on which nodes are malicious or selfish
	bool isMaliciousOrSelfish(string input);
	// ***************** END OCEAN ******************* //
};
